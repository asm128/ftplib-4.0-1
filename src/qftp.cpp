...
static int log_progress(netbuf *ctl, fsz_t xfered, void *arg)
{
	...
	printf("%s %5.2f%% %" PRIFSZ "\r", f->filename, pct, xfered);
    }
    else
    {
	printf("%s %" PRIFSZ "\r", f->filename, xfered);
    }
    fflush(stdout);
    return 1;
}
...
void process_file(char *filename)
{
	...
	if (action == FTP_SEND)
	{
	    ...
	    {
		if (!FtpMkdir(filename, conn))
		    fprintf(stderr,"mkdir %s failed\n%s",filename,FtpLastResponse(conn));
		else
		    if (ftplib_debug)
		...
		f->next = filelist;
		filelist = f;
		f->filename = _strdup(filename);
	    } else {
		netbuf *dir;
		char *buf;
		if (!FtpAccess(filename, FTPLIB_DIR, FTPLIB_ASCII, conn, &dir))
		{
		    fprintf(stderr,"error requesting directory of %s\n%s\n",
			    filename, FtpLastResponse(conn));
		    return;
		}
    ...
    {
      case FTP_DIR :
	sts = FtpDir(NULL, filename, conn);
	break;
      case FTP_LIST :
	sts = FtpNlst(NULL, filename, conn);
	break;
      case FTP_SEND :
	rem.next = NULL;
	...
	sts = FtpPut(filename,strippath ? basename(filename) : filename,mode,conn);
	if (ftplib_debug && sts)
	    printf("%s sent\n",filename);
	break;
      case FTP_GET :
	while (filelist)
	{
	    ...
	    filelist = f->next;
#if defined(__UINT64_MAX)
	    if (!FtpSizeLong(f->filename, &fsz, mode, conn))
#else
	    if (!FtpSize(f->filename, &fsz, mode, conn))
#endif
		fsz = 0;
	    f->fsz = fsz;
	    fsz /= 100;
	    ...
	    sts = FtpGet(f->filename,f->filename,mode,conn);
	    if (ftplib_debug && sts)
		printf("%s retrieved\n",f->filename);
	    free(f->filename);
	    free(f);
