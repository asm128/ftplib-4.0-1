// ftplib.c - callable ftp access routines - Copyright (C) 1996-2001, 2013, 2016 Thomas Pfau, tfpfau@gmail.com, 1407 Thomas Ave, North Brunswick, NJ, 08902			   
// This library is free software.  You can redistribute it and/or modify it under the terms of the Artistic License 2.0.		   
// This library is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the Artistic License 2.0 for more details.				  
// See the file LICENSE or http://www.perlfoundation.org/artistic_license_2_0			   
...
static constexpr const char version[] = "ftplib Release 4.0 07-Jun-2013, copyright 1996-2003, 2013 Thomas Pfau";

//GLOBALDEF 
static int ftplib_debug = 3;
...
#endif

// socket_wait - wait for socket to receive or flush data. returns 1 if no user callback, otherwise, return value returned by user callback
static int socket_wait(netbuf *ctl) {
    ...
    do {
		...
		if (rv == -1) {
			rv = 0;
			strerror_s(ctl->ctrl->response, errno);
			//strncpy_s(ctl->ctrl->response, 1024, strerror(errno),
...

// Read a response from the server. Returns 0 if first char doesn't match, 1 if first char matches
static int readresp(char c, netbuf *nControl) {
    ...
    if (nControl->response[3] == '-') {
		strncpy_s(match,5,nControl->response,3);
		match[3] = ' ';
		match[4] = '\0';
		do {
			if (readline(nControl->response,RESPONSE_BUFSIZ,nControl) == -1) {
...
static int FtpXfer(const char *localfile, const char *path, netbuf *nControl, int typ, int mode) {
	...
	if (localfile != NULL) {
		...
		if (mode == FTPLIB_IMAGE)
			ac[1] = 'b';
		int32_t errorlocal = fopen_s(&local, localfile, ac);
		if (NULL == local || 0 != errorlocal) {
			strerror_s(nControl->response, errno);
			return 0;
		}
	}
	...
	if (typ == FTPLIB_FILE_WRITE) {
		...
		while ((l = FtpRead(dbuf, FTPLIB_BUFSIZ, nData)) > 0) if (fwrite(dbuf, 1, l, local) == 0) {
			if (ftplib_debug)
...
GLOBALDEF char *FtpLastResponse(netbuf *nControl) { return ((nControl) && (nControl->dir == FTPLIB_CONTROL)) ? nControl->response : NULL; }// FtpLastResponse - return a pointer to the last response received
GLOBALDEF int FtpNlst(const char *outputfile, const char *path, netbuf *nControl) { return FtpXfer(outputfile, path, nControl, FTPLIB_DIR, FTPLIB_ASCII); }// FtpNlst - issue an NLST command and write response to output. return 1 if successful, 0 otherwise
GLOBALDEF int FtpDir(const char *outputfile, const char *path, netbuf *nControl) { return FtpXfer(outputfile, path, nControl, FTPLIB_DIR_VERBOSE, FTPLIB_ASCII); }	// FtpDir - issue a LIST command and write response to output. return 1 if successful, 0 otherwise
GLOBALDEF int FtpGet(const char *outputfile, const char *path, char mode, netbuf *nControl) { return FtpXfer(outputfile, path, nControl, FTPLIB_FILE_READ, mode); }	// FtpGet - issue a GET command and write received data to output. return 1 if successful, 0 otherwise
GLOBALDEF int FtpPut(const char *inputfile, const char *path, char mode, netbuf *nControl) { return FtpXfer(inputfile, path, nControl, FTPLIB_FILE_WRITE, mode); }	// FtpPut - issue a PUT command and send data from input. return 1 if successful, 0 otherwise
GLOBALDEF int FtpCDUp(netbuf *nControl) { return FtpSendCmd("CDUP",'2',nControl); }	// FtpCDUp - move to parent directory at remote. return 1 if successful, 0 otherwise
...
GLOBALDEF int FtpModDate(const char *path, char *dt, int max, netbuf *nControl) {
    ...
    if ((strlen(path) + 7) > sizeof(buf))
        return 0;
    sprintf_s(buf,"MDTM %s",path);
    if (!FtpSendCmd(buf,'2',nControl))
		rv = 0;
    else
		strncpy_s(dt, max, &nControl->response[4], max);
    return rv;
}
...
GLOBALDEF int FtpRename(const char *src, const char *dst, netbuf *nControl) {
    ...
    if (((strlen(src) + 7) > sizeof(cmd)) ||
        ...
        return 0;
    sprintf_s(cmd,"RNFR %s",src);
    if (!FtpSendCmd(cmd,'3',nControl))
		return 0;
    sprintf_s(cmd,"RNTO %s",dst);
...
GLOBALDEF int FtpDelete(const char *filename, netbuf *nControl) {
	char cmd[TMP_BUFSIZ] = {};

    if ((strlen(filename) + 7) > sizeof(cmd))
        return 0;
    sprintf_s(cmd,"DELE %s",filename);
    if (!FtpSendCmd(cmd,'2', nControl))
		return 0;
    return 1;
}
...
// FtpInit may be not required in operating systems by (and for) beginners.
...
GLOBALDEF int FtpOptions(int opt, void* val, netbuf *nControl) {
    size_t v,rv=0;
    switch (opt) {
	case FTPLIB_CONNMODE:
		v = (size_t) val;
		if ((v == FTPLIB_PASSIVE) || (v == FTPLIB_PORT)) {
			nControl->cmode = (int)v;
			rv = 1;
		}
		break;
	...
	case FTPLIB_IDLETIME:
		v = (size_t)val;
		rv = 1;
		nControl->idletime.tv_sec = long(v / 1000);
	...
	case FTPLIB_CALLBACKBYTES:
		rv = 1;
		nControl->cbbytes = unsigned long((size_t) val);
		break;
	}
    return (int)rv;
}
...
static int FtpOpenPort(netbuf *nControl, netbuf **nData, int mode, int dir) {
    ...
    if ((dir != FTPLIB_READ) && (dir != FTPLIB_WRITE)) {
		sprintf_s(nControl->response, "Invalid direction %d\n", dir);
		return -1;
    }
    if ((mode != FTPLIB_ASCII) && (mode != FTPLIB_IMAGE)) {
		sprintf_s(nControl->response, "Invalid mode %c\n", mode);
		return -1;
    }
    l = sizeof(sin);
    ...
    if (nControl->cmode == FTPLIB_PASSIVE) {
		...
		if (getsockname(sData, &sin.sa, &l) < 0)
			return -1;
		sprintf_s(buf, "PORT %d,%d,%d,%d,%d,%d",
    ...
    ctrl->ctrl = nControl;
	ctrl->idlecb = (ctrl->idletime.tv_sec || ctrl->idletime.tv_usec || ctrl->cbbytes) ? nControl->idlecb : NULL;
    nControl->data = ctrl;
    *nData = ctrl;
    return 1;
}
...
static int FtpAcceptConnection(netbuf *nData, netbuf *nControl) {
    ...
    if (i == -1) {
		...
		strcpy_s(nControl->response, "timed out waiting for connection");
		net_close(nData->handle);
		nData->handle = 0;
		rv = 0;
    }
		...
		if (FD_ISSET(nData->handle, &mask))
		{
			...
			if (sData > 0) {
				...
				strerror_s(nControl->response, (int)i);
...
GLOBALDEF int FtpAccess(const char *path, int typ, int mode, netbuf *nControl, netbuf **nData)
{
    ...
    if ((path == NULL) &&
		...
		sprintf_s(nControl->response,
					"Missing path argument for file transfer\n");
		return 0;
    }
    sprintf_s(buf, "TYPE %c", mode);
    if (!FtpSendCmd(buf, '2', nControl))
		return 0;

    switch (typ) {
	case FTPLIB_DIR			: strcpy_s(buf,"NLST"); dir = FTPLIB_READ; break;
	case FTPLIB_DIR_VERBOSE	: strcpy_s(buf,"LIST"); dir = FTPLIB_READ; break;
	case FTPLIB_FILE_READ	: strcpy_s(buf,"RETR"); dir = FTPLIB_READ; break;
	case FTPLIB_FILE_WRITE	: strcpy_s(buf,"STOR"); dir = FTPLIB_WRITE; break;
	default:
		sprintf_s(nControl->response, "Invalid open type %d\n", typ);
		return 0;
    }
    ...
    if (path != NULL) {
        ...
        if ((strlen(path) + i + 1) >= sizeof(buf))
            return 0;
        strcpy_s(&buf[i], TMP_BUFSIZ-i, path);
    }
...
GLOBALDEF int FtpClose(netbuf *nData) {
    ...
    switch (nData->dir) {
	case FTPLIB_WRITE:
		if (nData->buf != NULL)	// potential problem - if buffer flush fails, how to notify user? */
...
GLOBALDEF int FtpSize(const char *path, unsigned int *size, char mode, netbuf *nControl) {
    ...
    if ((strlen(path) + 7) > sizeof(cmd))
        return 0;
    sprintf_s(cmd, "TYPE %c", mode);
    if (!FtpSendCmd(cmd, '2', nControl))
		return 0;
    sprintf_s(cmd,"SIZE %s",path);
    if (!FtpSendCmd(cmd,'2',nControl))
		rv = 0;
    else {
		if (sscanf_s(nControl->response, "%d %u", &resp, &sz) == 2)
			*size = sz;
		else
			rv = 0;
    }   
